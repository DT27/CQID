    ...
    OpenIDAuthorizationCodeInterface
{
    ...
    public function setUser($username, $password, $email)
    {
        ...
        if ($this->getUser($username)) {
            $stmt = $this->db->prepare($sql = sprintf('UPDATE %s SET password=:password, email=:email where username=:username', $this->config['user_table']));
        } else {
            $stmt = $this->db->prepare(sprintf('INSERT INTO %s (username, password, email) VALUES (:username, :password, :email)', $this->config['user_table']));
        }
